...
const pJS = function(tag_id: string, params){
  ...
  pJS.fn.particle = function(color: { value: { r?: number, g?: number, b?: number, h?: number, s?: number, l?: number } | string }, opacity: number,  position?: { x: number, y: number }){
    ...
    let velbase: { x: number; y: number; } = { x: 0, y: 0 };
  ...
  pJS.fn.particle.prototype.draw = function() {
    ...
    //const p = this;
    let radius: number, opacity, color_value, img_obj: object;
    ...
    switch(this.shape){
      ...
      case 'circle':
        pJS.canvas.ctx.arc(this.x, this.y, radius, 0, Math.PI * 2, false);
      break;

      case 'edge':
        pJS.canvas.ctx.rect(this.x-radius, this.y-radius, radius*2, radius*2);
      break;

      case 'triangle':
        pJS.fn.vendors.drawShape(pJS.canvas.ctx, this.x-radius, this.y+radius / 1.66, radius*2, 3, 2);
      break;

      case 'polygon':
        pJS.fn.vendors.drawShape(
          pJS.canvas.ctx,
          this.x - radius / (pJS.particles.shape.polygon.nb_sides/3.5), // startX
      ...
      case 'star':
        pJS.fn.vendors.drawShape(
          pJS.canvas.ctx,
          this.x - radius*2 / (pJS.particles.shape.polygon.nb_sides/4), // startX
  ...
  pJS.fn.interact.linkParticles = function(p1: { x: number; y: number; }, p2: { x: number; y: number; }){
  ...
  pJS.fn.interact.attractParticles  = function(p1: { x: number; y: number; vx: number; vy: number; }, p2: { x: number; y: number; vx: number; vy: number; }){
  ...
  pJS.fn.interact.bounceParticles = function(p1: { x: number; y: number; radius: number; vx: number; vy: number; }, p2: { x: number; y: number; radius: number; vx: number; vy: number; }){
  ...
  pJS.fn.modes.pushParticles = function(nb: number, pos: { pos_x: number; pos_y: number; }){
  ...
  pJS.fn.modes.removeParticles = function(nb: number){
  ...
  pJS.fn.modes.bubbleParticle = function(p: { x: number; y: number; opacity_bubble: number | undefined; opacity: number; radius_bubble: number | undefined; radius: number; }){
    ...
    /* on click event */
      ...
      let dx_mouse, dy_mouse, dist_mouse: number = 0, time_spent: number = 0;
      ...
      const process = function(bubble_param: number, 
      ...
      if(pJS.tmp.bubble_clicking){
        /* size */
        process(pJS.interactivity.modes.bubble.size, pJS.particles.size.value, p.radius_bubble, p.radius, 'size', dist_mouse, time_spent);
        /* opacity */
        process(pJS.interactivity.modes.bubble.opacity, pJS.particles.opacity.value, p.opacity_bubble, p.opacity, 'opacity', dist_mouse, time_spent);
      }
  ...
  pJS.fn.modes.repulseParticle = function(p: { x: number; y: number; radius: number; vx: number; vy: number; vx_i: number; vy_i: number; }){
  ...
  pJS.fn.modes.grabParticle = function(p: { x: number; y: number; }){
  ...
  pJS.fn.vendors.eventsListeners = function(){
    ...
    if(pJS.interactivity.events.onhover.enable || pJS.interactivity.events.onclick.enable){
      ...
      /* el on mousemove */
      pJS.interactivity.el.addEventListener('mousemove', function(e: { clientX: number; clientY: number; offsetX: number; offsetY: number; }){
  ...
  pJS.fn.vendors.checkOverlap = function(p1: { x: number; y: number; radius: number; }, position: { x: number; y: number; }){
    for(let i = 0; i < pJS.particles.array.length; i++){
  ...
  pJS.fn.vendors.createSvgImg = function(p: { color: { rgb: { r: string; g: string; b: string; }; hsl: { h: string; s: string; l: string; }; }; opacity: string; img: { obj: HTMLImageElement; loaded: boolean; }; }){
  ...
  pJS.fn.vendors.drawShape = function(c: { 
    save: () => void; 
    beginPath: () => void; 
    ...
    fill: () => void; 
    restore: () => void; }, 
    startX: number, startY: number, sideLength: number, sideCountNumerator: number, sideCountDenominator: number){
  ...
  pJS.fn.vendors.loadImg = function(type: string){
...
export const particleJSUnload = function(tag_id: string){
  //clear canvas
  const canvas = document.getElementById(tag_id);
  canvas!.innerHTML = '';
}

particlesJS.load = function(tag_id: string, path_config_json: string | URL, callback: () => void) {
